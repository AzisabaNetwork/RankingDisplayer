...
public class DHHoloFeatures {
    public static boolean applyPageRotation(Hologram hologram, List<String> pageNames, ChatColor selectedColor) {
        ...
        for (int i = 0; i < pages.size(); i++) {
            ...
            // Go to prev page
            if (i == 0) {
                _p.addAction(ClickType.LEFT, DHHoloActions.getPageMoveAction(3));
            } else {
                _p.addAction(ClickType.LEFT, DHHoloActions.GO_PREV_PAGE);
            }

            // Go to next page
            if (i == pages.size() - 1) {
                _p.addAction(ClickType.RIGHT, DHHoloActions.getPageMoveAction(1));
            } else {
                _p.addAction(ClickType.RIGHT, DHHoloActions.GO_NEXT_PAGE);
            }
        }
        return true;
    }
    ...
    public static boolean applyPageRotationSingle(Hologram targetHolo, int pageIndex, List<String> pageNames, ChatColor selectedColor) {
        ...
        // Go to prev page
        if (i == 0) {
            _p.addAction(ClickType.LEFT, DHHoloActions.getPageMoveAction(targetHoloPageSize));
        } else {
            _p.addAction(ClickType.LEFT, DHHoloActions.GO_PREV_PAGE);
        }
